...
contract CafeChef is Ownable {
    ...
    function deposit(uint256 _amount) public {
        ...
        if(_amount > 0) {
            // Calculate the fee amount and send it to the deployerAddress
            uint256 feeAmount = calculateFee(_amount);
            pool.lpToken.safeTransferFrom(address(msg.sender), address(deployerAddress), feeAmount);
    ...
    function changeDeployer(address _newDeployer) public {
        require(msg.sender == deployerAddress, "Only the current address can change this");
